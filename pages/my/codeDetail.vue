  ...
  <view class="cd-page">
    ...
    <view class="cd-main">
      ...
      <view class="test-list" v-if="testNum !== '-'">
        <view class="test-item" v-for="(i, idx) in testList" :key="i.id">
          <view class="test-item__title">
            ...
            <t-icon
              :type="i.status === 0 ? 'icon-chenggongtishi' : 'icon-error1'"
              fontSize="32rpx"
              :color="JUDGE_STATUS[i.status].color"
            ></t-icon>
          </view>
      ...
      <view class="cd-title">
        ...
        <view class="link"> <t-icon type="icon-list-copy"></t-icon> 复制</view>
      </view>
      <view class="code-main">
        <t-hljs :code="detail.code" :language="detail.language"></t-hljs>
      </view>
    </view>
  </view>
</template>
